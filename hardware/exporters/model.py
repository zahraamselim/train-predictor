"""
Export ML models (ETA + ETD) to Arduino C headers
...
def export_models(eta_path='outputs/models/eta_model.pkl',
                  etd_path='outputs/models/etd_model.pkl',
                  output_path='hardware/eta_model.h'):
    """Export both ETA and ETD models"""
    
    Logger.section("Exporting ML models to Arduino")
    
    eta_exists = Path(eta_path).exists()
    etd_exists = Path(etd_path).exists()
    
    if not eta_exists and not etd_exists:
        Logger.log("WARNING: No trained models found")
        Logger.log("Run: make ml-pipeline")
        Logger.log("Generating physics fallback instead")
        export_physics_fallback(output_path)
        return True
    
    if eta_exists:
        Logger.log(f"Found ETA model: {eta_path}")
    if etd_exists:
        Logger.log(f"Found ETD model: {etd_path}")
    
    Logger.log("")
    Logger.log("NOTE: Full Gradient Boosting models (~120KB each) too large for Arduino Uno")
    Logger.log("Using physics-based fallback calculations")
    Logger.log("For full ML: Use Arduino Mega (256KB) or ESP32 (4MB)")
    
    export_physics_fallback(output_path)
    return True


def export_physics_fallback(output_path):
    """Export physics-based ETA and ETD calculations"""
...
"""
    ...
    Logger.log("Using physics calculations (Arduino Uno compatible)")


if __name__ == '__main__':
    import argparse
    
    parser = argparse.ArgumentParser(description='Export ML models to Arduino')
    parser.add_argument('--eta', default='outputs/models/eta_model.pkl',
                       help='ETA model pickle file')
    parser.add_argument('--etd', default='outputs/models/etd_model.pkl',
                       help='ETD model pickle file')
    parser.add_argument('--output', default='hardware/eta_model.h',
                       help='Output C header file')
    args = parser.parse_args()
    
    export_models(args.eta, args.etd, args.output)