...
class RailwayGate:
    ...
    def draw(self, surface):
        pole_x = self.x - 35 if self.side == 'left' else self.x + 35
        ...
        for i in range(0, self.barrier_length, stripe_width * 2):
            pygame.draw.rect(barrier_surface, self.colors['stripe_yellow'],
                           (i, 0, stripe_width, self.barrier_width))
            pygame.draw.rect(barrier_surface, self.colors['stripe_black'],
                           (i + stripe_width, 0, stripe_width, self.barrier_width))
        
        angle_rad = math.radians(-self.angle if self.side == 'left' else self.angle)
...
class CountdownTimer:
    ...
    def _draw_digit(self, surface, x, y, digit):
        ...
        seg_positions = [
            (x + 2, y, 'horizontal'),
            (x + 12, y + 2, 'vertical_top'),
            (x + 12, y + 12, 'vertical_top'),
            (x + 2, y + 20, 'horizontal'),
            (x, y + 12, 'vertical_top'),
            (x, y + 2, 'vertical_top'),
            (x + 2, y + 10, 'horizontal')
    ...
    def draw(self, surface):
        ...
        for i, digit in enumerate(time_str):
            ...
            if i == 1:
                pygame.draw.circle(surface, self.colors['segment_on'], 
                                 (digit_x + 16, self.y - 3), 2)
                pygame.draw.circle(surface, self.colors['segment_on'], 
                                 (digit_x + 16, self.y + 3), 2)


class RoadIntersectionSignal:
    ...
    def draw(self, surface):
        ...
        if self.position == 'top':
            signal_x, signal_y = self.x, self.y - offset
        elif self.position == 'bottom':
            signal_x, signal_y = self.x, self.y + offset
        elif self.position == 'left':
            signal_x, signal_y = self.x - offset, self.y
        else:
            signal_x, signal_y = self.x + offset, self.y
        
        housing_rect = pygame.Rect(
            signal_x - self.housing_size // 2,
            signal_y - self.housing_size // 2,
            self.housing_size,
            self.housing_size
        )
        pygame.draw.rect(surface, self.colors['housing'], housing_rect, border_radius=5)
        pygame.draw.rect(surface, (30, 30, 35), housing_rect, 2, border_radius=5)
        
        show_light = True
        if 'blink' in self.state:
            show_light = self.blink_visible
        
        if self.state.startswith('red') and show_light:
            pygame.draw.circle(surface, self.colors['red_on'], (signal_x, signal_y), self.light_radius)
            pygame.draw.circle(surface, (255, 100, 100), (signal_x, signal_y), self.light_radius - 2)
        elif self.state == 'green':
            pygame.draw.circle(surface, self.colors['green_on'], (signal_x, signal_y), self.light_radius)
            pygame.draw.circle(surface, (100, 255, 100), (signal_x, signal_y), self.light_radius - 2)
        else:
            color = self.colors['red_off'] if self.state.startswith('red') else self.colors['green_off']
            pygame.draw.circle(surface, color, (signal_x, signal_y), self.light_radius)
