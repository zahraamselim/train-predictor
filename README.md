...
- 3 IR Sensors: Detect the train at 50cm, 40cm, 30cm from crossing
- Servo Motor: Controls the crossing gate
- TM1637 Display: Shows countdown time in MM:SS format
- 4 LEDs: Crossing (green/red) and Intersection (green/red)
- Buzzer: Warning sound with pattern alerts

### Operation Sequence

1. Detection Phase: Train passes three sensors spaced 10cm apart
   - Sensor 0 triggers at time t0 (furthest from crossing)
   - Sensor 1 triggers at time t1 (middle position)
   - Sensor 2 triggers at time t2 (nearest to crossing)

2. Calculation Phase: System extracts 14 features from timestamps
   - Time intervals between sensors
   - Calculated speeds in each section
...
From three sensor timestamps and known geometry, we calculate 14 features:
...
Feature Categories:
- Temporal (1-3): Direct timing measurements from sensors
- Geometric (4-6): Distances defining the system layout
- Kinematic (7-10): Velocity and acceleration calculations
- Physical Constants (11-12): Train geometry
- Baseline Predictions (13-14): Simple physics estimates that ML learns to correct

Why Include Baseline Predictions?

Features 13 and 14 give the model a starting point using simple constant-speed physics. The Random Forest then learns patterns like "when eta_baseline = 3.0s but acceleration is negative, actual ETA is usually 3.4s." This is called residual learning where the model corrects a simple estimate rather than predicting from scratch.
...
3. Final prediction equals the average of all trees
...
Why Random Forest for ETA/ETD?

Random Forest Regressor (not Classifier) predicts continuous numerical values, not categories. Each tree outputs a number (like 3.21 seconds), and averaging 10 trees gives precise continuous predictions. This is different from Random Forest Classifier which would predict discrete categories like "fast/medium/slow."
...
R² (coefficient of determination) measures prediction accuracy from 0 to 1:
- R² = 1.0 means perfect predictions
- R² = 0.5 means model explains 50% of variation
- R² = 0.0 means model no better than guessing average

Our ETA Model R² = 0.986 means it explains 98.6% of variation in train arrival times. Only 1.4% is unexplained due to sensor noise or random factors.

What is Mean Absolute Error?

MAE is the average error across all predictions:
```
MAE = (|actual1 - predicted1| + |actual2 - predicted2| + ... + |actualN - predictedN|) / N
```

Our ETA MAE of 0.031 seconds means predictions are off by only 31 milliseconds on average.

Why Better Than Simple Physics?

Physics baseline assumes constant speed:
```
ETA = distance / speed = 0.3m / 0.1m/s = 3.0s
```

Reality when train is decelerating at 0.05 m/s²:
```
Actual ETA = 3.4s  (physics was wrong by 0.4s)
```

Our ML Model learned from 2000 examples that trains with this deceleration pattern take 13% longer:
```
...
- 1x Breadboard
- Jumper wires
- 4x AA Batteries (for servo external power)
...
Power Note: Servo motor requires external 5V (4x AA batteries) for reliable operation. Connect servo GND to Arduino GND for common ground.
...
Generates 2000 train trajectories (takes about 5 minutes). Output: outputs/features.csv
...
Trains Random Forest models (takes about 1 minute). Outputs: outputs/eta_model.pkl, outputs/etd_model.pkl
...
Full simulation takes about 30 minutes. Output: outputs/comparison.json
...
Runs training, simulation, and Arduino export.
...
3. Expected speed: approximately 8 cm/s
...
void loop() {
    ...
    // Trigger only on HIGH to LOW transition (falling edge)
...
For changing speed (acceleration not zero):

Using kinematic equation: d = v0*t + 0.5*a*t²
...
The trained Random Forest models are too large for Arduino Uno (32KB total memory). Each decision tree stores split conditions, tree structure, and leaf values. Our 10-tree ETA model would need about 50KB.

Solution: Use physics-based approximations on Arduino that mimic the trained model behavior. While not as accurate as the full Random Forest (which achieves 0.031s error), the physics approximation achieves about 0.15s error, which is sufficient for the demo system.

For real deployment, use a more powerful microcontroller like ESP32 (520KB RAM) that can run the full Random Forest model.
...
| 3x IR Sensors | 5V | 20mA each | 0.30W |
| 4x LEDs | 5V | 20mA each | 0.40W |
| Buzzer | 5V | 30mA | 0.15W |
| Servo Motor | 5V | 500mA | 2.50W |
| TM1637 Display | 5V | 80mA | 0.40W |
| Total | | | 6.65W |

Can be powered by USB (Arduino) + 4x AA batteries (servo).
...
- Move hand/object at approximately 8 cm/s
...
Travel Time: 49.2% reduction (180s to 91.5s)
Queue Length: 45.2% reduction (60 to 33 vehicles)
Fuel Consumption: 0.5% reduction
CO2 Emissions: 0.5% reduction
