...
class DataCollector:
    ...
    def run(self, duration=3600):
        traci.start(['sumo', '-c', 'sumo/complete/simulation.sumocfg', '--start', '--quit-on-end'])
        
        Logger.log(f"Starting data collection for {duration}s")
    ...
    def _track_vehicles(self, t):
        for vid in traci.vehicle.getIDList():
            ...
            is_at_crossing = (abs(pos - self.crossing_w) < 50 or abs(pos - self.crossing_e) < 50)
            
            if is_at_crossing:
                if vid not in self.vehicles_at_crossing:
                    self.vehicles_at_crossing[vid] = {'enter_time': t, 'enter_speed': speed}
    ...
    def _track_trains(self, t):
        for tid in traci.vehicle.getIDList():
            ...
            if tid not in self.trains:
                self.trains[tid] = {'speeds': [], 'positions': [], 'times': [], 'length': length}
