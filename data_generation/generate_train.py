...
def simulate_train_approach(train_type, initial_speed, grade, weather, 
    ...
    while distance > -(train_length + 50):
        ...
        speed_ms = max(0, speed_ms + accel * dt)
        ...
        events = sensors.update(distance, time, train_length)
    ...
    if None in [sensor_0_entry, sensor_1_entry, sensor_2_entry]:
        return None
    ...
    speed_0_to_1 = (dist_0_to_1 / time_0_to_1) * 3.6 if time_0_to_1 > 0 else 0
    speed_1_to_2 = (dist_1_to_2 / time_1_to_2) * 3.6 if time_1_to_2 > 0 else 0
    
    for point in trajectory:
        if point['time'] >= sensor_2_entry and point['distance'] <= 0:
            eta = point['time'] - sensor_2_entry
            break
    else:
        eta = sensor_positions[2] / (speed_1_to_2 / 3.6) if speed_1_to_2 > 0 else 0
...
def generate_train_dataset(num_scenarios=100, output_file=None):
    """Generate train approach dataset."""
    ...
    print(f"\nGenerating {num_scenarios} train approach scenarios ({scale_mode} scale)")
